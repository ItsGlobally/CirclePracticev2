    ...
    public static void updateScoreboard(Player p) {
        ...
        switch (pp.getState()) {
            ...
            case PlayerState.SPECTATING -> {
                ...
                if (game.getKit().isRespawnable()) {
                    lines.add("&fRed's bed: " + (game.getPlayerRespawnable(p1) ? "&d✓" : "&c✗"));
                    lines.add("&fBlue's bed: " + (game.getPlayerRespawnable(p2) ? "&d✓" : "&c✗"));
                    lines.add("&d&f");
                }
                if (game.getKit().isCountHit()) {
                    lines.add("&fRed's hits&d: " + game.getPlayerhit(p1) + "&f/&d" + game.getKit().getCountHitToDie());
                    lines.add("&fBlue's hits&d: " + game.getPlayerhit(p2) + "&f/&d" + game.getKit().getCountHitToDie());
                    lines.add("&d&f");
                }
                lines.add("&fRed's ping&d: " + NMSUtils.getPing(p1.getPlayer()));
                lines.add("&fBlue's ping&d: " + NMSUtils.getPing(p2.getPlayer()));
                lines.add("&f");
            }
        }
